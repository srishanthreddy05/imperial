...
export async function saveAIConversation(
  ...
  const payload: AIConversationDocument = {
    ...
    emailSent: memory.emailSent ?? existing?.emailSent ?? false,
    intakeMode: memory.intakeMode ?? false,
    detailsRequested: memory.detailsRequested ?? false,
    callbackRequested: memory.callbackRequested ?? false,
    callbackSubmitted: memory.callbackSubmitted ?? false,
